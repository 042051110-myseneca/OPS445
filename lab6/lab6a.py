...
class Student:
    ...
    def displayCourses(self):
        if 0.0 in self.courses.values():
            removefailed = {key:val for key,val in self.courses.items() if val != 0.0}
            courseslist = list(removefailed)
            return courseslist
        else:
            courseslist = list(self.courses.keys())
            return courseslist
